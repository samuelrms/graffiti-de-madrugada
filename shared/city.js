// Deterministic city generator shared by server (CommonJS) and browser (ESM).
// Same seed => same buildings on both sides, so the server can validate paint.

const SEED = 20260821;
const BLOCKS = 6; // blocks per side
const BLOCK = 24; // block size (units)
const STREET = 8; // street width
const CELL = BLOCK + STREET;
const MAP_SIZE = BLOCKS * CELL + STREET;
const TILE = 2; // paint tile size on walls
...
// Building: axis-aligned box. x,z = center; w,d = footprint; h = height.
// kind: 'tower' | 'house' | 'shop'. Houses get a roof (visual only).
...
function tileCenter(b, face, i, j) {
...
function tileKey(bi, face, i, j) {
  return `${bi}:${face}:${i}:${j}`;
}

function parseKey(key) {
  const p = key.split(':').map(Number);
  return p.length === 4 && p.every(Number.isInteger) ? { bi: p[0], face: p[1], i: p[2], j: p[3] } : null;
}

// Points for a tile: higher = more (reason to climb).
function tileValue(j) {
  return 1 + Math.floor((j * TILE) / 6);
}

const api = { SEED, BLOCKS, BLOCK, STREET, CELL, MAP_SIZE, TILE, generateCity, spawnPoints, pickupPoints, faceInfo, tileCenter, tileKey, parseKey, tileValue };

if (typeof module !== 'undefined') module.exports = api;
if (typeof window !== 'undefined') window.CITY = api;
