...
export type BuildingKind = 'tower' | 'house' | 'shop';
// Axis-aligned box. x,z = center; w,d = footprint; h = height. Houses get a roof (visual only).
export interface Building { kind: BuildingKind; x: number; z: number; w: number; d: number; h: number; hue: number }
export interface Point { x: number; z: number }
export type PickupType = 'vest' | 'medkit' | 'shoes' | 'doublecan' | 'bazooka';
export interface PickupPoint { type: PickupType; x: number; y: number; z: number; roof?: number }
export type Face = 0 | 1 | 2 | 3; // +x, -x, +z, -z
export interface TileRef { bi: number; face: Face; i: number; j: number }
...
export function pickupPoints(): PickupPoint[] {
  const used = new Set(spawnPoints().map((s) => `${s.x},${s.z}`));
  const types: PickupType[] = ['vest', 'medkit', 'shoes', 'doublecan', 'bazooka'];
  ...
  city.forEach((b, bi) => {
    if (b.kind === 'tower' && rnd() < 0.8) out.push({ type: 'bazooka', x: b.x, y: b.h, z: b.z, roof: bi });
    else if (b.kind === 'shop' && rnd() < 0.5) out.push({ type: rnd() < 0.5 ? 'vest' : 'medkit', x: b.x, y: b.h, z: b.z, roof: bi });
  });
  return out;
}
...
export function faceInfo(b: Building, face: Face): { cols: number; rows: number } {
  const along = face < 2 ? b.d : b.w;
  return { cols: Math.floor(along / TILE), rows: Math.floor(b.h / TILE) };
}

export interface TileCenter { x: number; y: number; z: number; nx: number; nz: number }
export function tileCenter(b: Building, face: Face, i: number, j: number): TileCenter {
...
export function tileKey(bi: number, face: Face, i: number, j: number): string {
  return `${bi}:${face}:${i}:${j}`;
}

export function parseKey(key: string): TileRef | null {
  const p = key.split(':').map(Number);
  if (p.length !== 4 || !p.every(Number.isInteger) || p[1] < 0 || p[1] > 3) return null;
  return { bi: p[0], face: p[1] as Face, i: p[2], j: p[3] };
}

// Points for a tile: higher = more (reason to climb).
export function tileValue(j: number): number {
